...
def barrier_all(group, rank, subset=False):
    global barrier_time
...
p2p_group_dict = None
def p2p_broad_func(node_count, am_partitions, inputs, rank, size, group):
    global p2p_group_dict
    global device
    global comm_time
    global comp_time
    global scomp_time
...
def run(rank, size, inputs, adj_matrix, adj_values,data, features, classes, device):
    ...
    with open('o1d%d_100_epochs.txt'%(rank), 'wt') as f:
        print(f"rank: {rank} median_run: {median_idx}", file=f)
        print(f"rank: {rank} total_time: {total_time[median_idx][rank]}", file=f)
        print(f"rank: {rank} comm_time: {comm_time[median_idx][rank]}", file=f)
        print(f"rank: {rank} comp_time: {comp_time[median_idx][rank]}", file=f)
        print(f"rank: {rank} scomp_time: {scomp_time[median_idx][rank]}", file=f)
        print(f"rank: {rank} dcomp_time: {dcomp_time[median_idx][rank]}", file=f)
        print(f"rank: {rank} bcast_comm_time: {bcast_comm_time[median_idx][rank]}", file=f)
        print(f"rank: {rank} barrier_time: {barrier_time[median_idx][rank]}", file=f)
        print(f"rank: {rank} op1_comm_time: {op1_comm_time[median_idx][rank]}", file=f)
        print(f"rank: {rank} op2_comm_time: {op2_comm_time[median_idx][rank]}", file=f)
        print(f"rank: {rank} {outputs}", file=f)
    ...
    if accuracy:
        ...
        print(log.format(100, train_acc, best_val_acc, test_acc))
...
if __name__ == '__main__':
    ...
    parser.add_argument("--accperrank", type=int, default=8)
    parser.add_argument("--epochs", type=int, default=100)
    parser.add_argument("--graphname", type=str)
    parser.add_argument("--timing", type=str, default='True')
    parser.add_argument("--midlayer", type=int, default=16)
