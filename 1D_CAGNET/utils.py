...
class DistData(DistUtil):
    def __init__(self, env, graph_name):
        ...
        self.partition_1d()
        print('Rank',rank,'data ready')

    def partition_1d(self):
        self.local_features, self.local_adj, self.local_adj_parts = \
            DistData.CANNET_oned(self.rank, self.world_size, self.features, self.adj_indices, self.adj_values, self.features)
        self.local_features = self.local_features.to(self.device)
        self.local_adj = self.local_adj.to(self.device)
        for i in range(self.world_size:
            self.local_adj_parts[i] = self.local_adj_parts[i].t().coalesce().to(device)
    ...
    def CAGNET_oned(rank, world_size, inputs, adj_matrix, adj_values, features):
        ...
        n_per_proc = math.ceil(float(node_count) / world_size)
        with torch.no_grad():
            ...
            input_partitions = torch.split(inputs, math.ceil(float(inputs.size(0)) / world_size), dim=0)
