...
def outer_product2(inputs, ag):
    ...
    dist.all_reduce(grad_weight, op=dist.ReduceOp.SUM, group=g_env.col_groups[g_env.rank_col])
    torch.cuda.synchronize()
...
def broad_func(node_count, am_partitions, inputs, btype=None):
    ...
    device = g_env.device
    n_per_proc = math.ceil(float(node_count) / g_data.data_total_parted)
    ...
    basic_stages = g_env.world_size//(g_env.rep**2)
    stages = basic_stages
    if g_env.rank_col == g_env.rep-1:  # last node in group
        stages = (g_env.world_size // g_env.rep) - (g_env.rep - 1) * stages

    for i in range(stages):
        am_partid = g_env.rank_col * basic_stages + i
        q = am_partid * g_env.rep + g_env.rank_col
        q_c = q // g_env.rep

        if q == g_env.rank:
            inputs_recv = inputs.clone()
        elif q_c == g_data.data_total_parted - 1:
            inputs_recv = torch.zeros((am_partitions[am_partid].size(1), inputs.size(1)), device=device)

        g_timer.barrier_all()
        torch.cuda.synchronize()

        g_timer.start('broadcast')
        dist.broadcast(inputs_recv, src=q, group=g_env.col_groups[g_env.rank_col])
    ...
    dist.all_reduce(z_loc, op=dist.reduce_op.SUM, group=g_env.row_groups[g_env.rank_grp])
...
def main():
    ...
    for i in range(args.run_count):
        ...
        local_train_mask = torch.split(g_data.g.train_mask.bool(), am_pbyp[0].size(0), dim=0)[g_data.data_rank]
        local_labels = torch.split(g_data.g.labels, am_pbyp[0].size(0), dim=0)[g_data.data_rank]
...
if __name__ == '__main__':
    ...
    print(args)
    g_env = utils.DistEnv(args.local_rank, args.world_size, args.rep, args.backend)
